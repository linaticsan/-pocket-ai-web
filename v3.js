...
function setupDocumentIntelligence(){const files=$v('files');if(!files||$v('v3DocTools'))return;const tools=document.createElement('div');tools.id='v3DocTools';tools.className='v3-doc-tools document-studio-v50';tools.innerHTML=`<div class="doc-studio-head"><div><p class="eyebrow">DOCUMENT STUDIO</p><strong>Work with files here</strong><small>Document AI, study tools and creation now live in Documents — Library stays focused on books.</small></div><span>✨</span></div><div class="doc-studio-grid"><button id="v3DocSummary">📝<strong>Summarize</strong><small>Key points</small></button><button id="v3DocExplain">💡<strong>Explain</strong><small>Simple language</small></button><button id="v3DocQuiz">🎓<strong>Quiz</strong><small>Questions + answers</small></button><button id="v3DocStudy">📚<strong>Study guide</strong><small>Learn step by step</small></button><button id="v3DocSlides">🖥️<strong>Slides</strong><small>Presentation outline</small></button><button id="v3DocFlash">🗂️<strong>Flashcards</strong><small>Q / A cards</small></button></div><button id="v3DocChat" class="primary doc-studio-chat">Ask about this document in Chat →</button>`;files.querySelector('.file-tools')?.after(tools);const send=(prefix)=>{const text=$v('fileText').value.trim();if(!text)return alert('Open or write a document first.');document.querySelector('[data-go="chat"]')?.click();$v('prompt').value=prefix+'\n\n'+text.slice(0,30000);$v('prompt').focus()};$v('v3DocSummary').onclick=()=>send('Summarize this document with clear headings and key points:');$v('v3DocExplain').onclick=()=>send('Explain this document simply and clearly. Define difficult terms and use examples:');$v('v3DocQuiz').onclick=()=>send('Create a useful study quiz from this document. Put a separate answer key after the questions:');$v('v3DocStudy').onclick=()=>send('Create a structured study guide from this document with key concepts, vocabulary, examples, review questions and answers:');$v('v3DocSlides').onclick=()=>send('Turn this document into a presentation outline. Use slide titles followed by concise bullet points:');$v('v3DocFlash').onclick=()=>send('Create flashcards from this document. Format each as Q: then A:');$v('v3DocChat').onclick=()=>send('Use this document as context and help me with it:')}
