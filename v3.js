...
function setupChatUI(){const chat=$v('chat');if(!chat||$v('v3ChatShell'))return;const messages=$v('messages'),form=$v('chatForm');if(!messages||!form){console.warn('Pocket AI chat UI could not initialize: base chat elements are missing.');return}chat.querySelector('.hero')?.remove();chat.querySelector('.engine-row')?.remove();const shell=document.createElement('div');shell.id='v3ChatShell';shell.className='v3-chat-shell';shell.innerHTML=`<aside class="v3-sidebar" id="v3HistoryPanel" aria-label="Chat history"><div class="v3-side-head"><strong>Chats</strong><div class="v3-side-actions"><button id="v3NewChat" type="button" aria-label="Start a new chat">＋</button><button id="v3HistoryClose" type="button" aria-label="Close chat history">×</button></div></div><div id="v3ChatList"></div></aside><div class="v3-chat-main"><div class="v3-chat-top"><div><p class="eyebrow">POCKET AI V3</p><strong id="v3ChatTitle">Unified conversation</strong></div><div class="v3-top-controls"><button id="v3Api" title="AI connections" aria-label="AI connections"><span class="v84-mini-icon" data-icon="spark"></span><small>AI</small></button><button id="v3Study" title="Study mode" aria-label="Study mode"><span class="v84-mini-icon" data-icon="study"></span><small>Study</small></button><button id="v3Library" title="Library context" aria-label="Library context"><span class="v84-mini-icon" data-icon="book"></span><small>Books</small></button><button id="v3MobileHistory" type="button" title="Chat history" aria-label="Open chat history" aria-controls="v3HistoryPanel" aria-expanded="false"><span class="v84-mini-icon" data-icon="menu"></span><small>History</small></button></div></div></div>`;chat.insertBefore(shell,messages);shell.querySelector('.v3-chat-main').append(messages,form);form.classList.add('v3-composer');form.innerHTML=`<div id="v3Attachments" class="v3-attachments"></div><div class="v3-input-row"><input id="v3Attach" type="file" multiple hidden accept=".txt,.md,.json,.csv,.pdf,.docx,.pptx,text/*,application/pdf"><button type="button" id="v3AttachBtn" aria-label="Attach files">＋</button><textarea id="prompt" rows="1" maxlength="40000" placeholder="Talk to Pocket AI…" required></textarea><button class="primary" id="chatSend">Send</button></div><div class="v3-compose-meta"><span id="v3RouteBadge">✨ Auto routing</span><button type="button" id="v3Stop" hidden>Stop</button></div>`;$v('clearChat')?.remove();$v('v3NewChat').onclick=newChat;$v('v3Api').onclick=()=>{if(window.PocketAPI?.open)window.PocketAPI.open();else $v('aiDialog')?.showModal()};$v('v3Study').onclick=()=>{const on=$v('v3Study').classList.toggle('active');$v('prompt').placeholder=on?'Ask what you want to learn…':'Talk to Pocket AI…';const n=$v('notice');if(n)n.textContent=on?'🎓 Study Mode on':'Study Mode off'};$v('v3Library').onclick=()=>{const on=$v('v3Library').classList.toggle('active');$v('prompt').placeholder=on?'Ask your saved Library books…':'Talk to Pocket AI…';const n=$v('notice');if(n)n.textContent=on?'📚 Library context on':'Library context off'};const setHistoryOpen=on=>{shell.classList.toggle('show-sidebar',!!on);$v('v3MobileHistory')?.setAttribute('aria-expanded',on?'true':'false');if(on)requestAnimationFrame(()=>$v('v3HistoryClose')?.focus({preventScroll:true}))};$v('v3MobileHistory').onclick=()=>setHistoryOpen(!shell.classList.contains('show-sidebar'));$v('v3HistoryClose').onclick=()=>{setHistoryOpen(false);$v('v3MobileHistory')?.focus({preventScroll:true})};shell.addEventListener('keydown',e=>{if(e.key==='Escape'&&shell.classList.contains('show-sidebar')){e.preventDefault();setHistoryOpen(false);$v('v3MobileHistory')?.focus({preventScroll:true})}});$v('v3AttachBtn').onclick=()=>$v('v3Attach').click();$v('v3Attach').onchange=handleAttachments;$v('v3Stop').onclick=()=>{V3.stop=true;$v('v3Stop').textContent='Stopping…'};form.onsubmit=sendV3;$v('prompt').onkeydown=e=>{if(e.key==='Enter'&&!e.shiftKey&&!e.isComposing){e.preventDefault();form.requestSubmit($v('chatSend'))}};routeBadge();renderConversation()}
function currentModelLabel(){
 if(window.PocketAPI?.hasKeys?.()){
  const saved=localStorage.getItem('pocket-api-provider')||'';
  const model=localStorage.getItem('pocket-api-model')||localStorage.getItem('geminiModel')||'';
  return model?('Cloud AI · '+model):(saved?('Cloud AI · '+saved):'Smart routing');
 }
 const model=window.PocketLocalAI?.model||localStorage.getItem('pocket-local-model')||'';
 const ready=window.PocketLocalAI?.isConnected?.();
 return (ready?'● ':'○ ')+'Local AI'+(model?' · '+model:'');
}
function routeBadge(){const b=$v('v3RouteBadge');if(!b)return;b.textContent=currentModelLabel()}
