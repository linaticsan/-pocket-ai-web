...
function setProgress(value,text){
 const p=el('localProgress');if(!p)return;
 p.hidden=false;p.value=Math.max(0,Math.min(1,Number(value)||0));
 const t=el('localProgressText');if(t)t.textContent=text||'Preparing Local AI…';
...
async function initLocal(){
 const dev=el('localDevice'),compat=el('localCompatibility');
 if(dev)dev.textContent=deviceName();
 const supported=await webgpuReady();
 if(compat)compat.textContent=supported?'WebGPU available ✓':'WebGPU unavailable';
 if(!supported){
   setStatus('This browser cannot run the built-in Local AI yet. Use a current WebGPU-capable browser or the Advanced server connection below.','error');
   if(el('localSetup'))el('localSetup').disabled=true;
   if(el('localConnect'))el('localConnect').disabled=true;
   return;
 }
 controls(false);
 const installed=saved(LOCAL_INSTALLED)==='1';
 if(installed){
   setStatus('Local AI was previously installed. Press Connect to load the saved model.','idle');
 }else{
   setStatus('Ready for first-time setup. Setup downloads the model once and saves it in this browser.','idle');
 }
}
