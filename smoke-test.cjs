...
assert(index.includes("const BUILD='step27-runtime-cleanup'"),'STEP 27 build marker is missing');
...
assert(sw.includes("const CACHE=APP_CACHE_PREFIX+'v32'"),'STEP 27 app-shell cache must be v32');
...
assert(index.includes('./workspace.js?v=20260926-step27-runtime-cleanup'),'STEP 27 workspace cache-bust is missing');
...
assert(workspace.includes("const POCKET_BUILD='step27-runtime-cleanup'"),'STEP 27 diagnostics build marker is missing');
...
assert(index.includes('<dd>STEP 27</dd>'),'Visible diagnostics build label is stale');
